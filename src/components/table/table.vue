<template>
    <div :class="wrapClasses" :style="styles">
        <div :class="classes" ref="table"
             v-on="tableEvents"
             @dblclick.capture="handTableDoubleclick">
        ...
        <div :class="[prefixCls + '-resize-line']" :style="resizeLineStyle" v-show="isOnResizeCol"></div>
        <Spin fix size="large" v-if="loading || localeLoading">
            <slot name="loading"></slot>
        </Spin>
        <div :ref="prefixCls + '-cell-tooltip'"
            ...
            tableEvents() {
                ...
                if (!this.isEditing) {
                    Object.assign(events, {
                        mousemove: this.handTableMousemove,
                        mouseleave: this.handTableMouseleave,
                        mouseup: this.handTableMouseup,
                        mousedown: this.handTableMousedown,
                    });
                }
                return events;
            },
...
//                    const width = this.bodyHeight === 0 ? this.tableWidth : this.tableWidth - this.scrollBarWidth;
        ...
        methods: {
            ...
            /* 新增 */
            handTableDoubleclick(evt) {
                // console.log(1);
            },
            handTableMousemove(evt) {
                this.handleTHeadMousemove(evt);
                this.handleTooltipIn(evt);
            },
            handTableMouseleave(evt) {
                this.resetResizeCol(evt);
            },
            handTableMouseup(evt) {
                this.handleTHeadMouseup(evt);
            },
            handTableMousedown(evt) {
                this.handleTHeadMousedown(evt);
            },
            /* tooltip相关*/
            setTooltipPos(evt) {
            ...
            handleTooltipIn(evt) {
                if (this.isEnableTooltip(evt.target)) {
                    this.setTooltipPos(evt);
                } else {
                    this.isShowTooltip = false;
                }
            },
            /* 表头拖动相关 */
            toggleCursor(cursorType = 'default') {
                document.documentElement.style.cursor = cursorType;
                document.documentElement.style.userSelect = cursorType === 'default' ? 'auto' : 'none';
            },
            getOffsetWithClient(dom, direction) {
                const offset = dom.getBoundingClientRect();
                return direction ? offset[direction] || 0 : offset;
            },
            isFixedRightTh(th) {
                return th.dataset.fixed === 'right';
            },
            getTh(dom) {
                let count = 5, flag = false;
                while (count && !flag) {
                    if (dom.tagName.toLocaleLowerCase() !== 'th') {
                        dom = dom.parentNode;
                    } else {
                        flag = true;
                    }
                ...
                return flag ? dom : null;
            },
            getThOpt(th) {
                const thTitle = th.dataset.title;
                return this.cloneColumns.find(item => item.title === thTitle);
            },
            setResizeLinePos(evt) {
                const tableOffsetLeft = this.getOffsetWithClient(this.$refs['body'], 'left');
                this.resizeLineStyle.left = evt.clientX - tableOffsetLeft + 'px';
            },
            resetResizeCol() {
                this.resizeColTarget = null;
                this.canResizeCol = this.isOnResizeCol = false;
                this.toggleCursor();
            },
            handleTHeadMouseup(evt) {
                if (this.canResizeCol && this.isOnResizeCol && this.resizeColTarget) {
                    this.localeLoading = true;
                    const thOffsetLeft = this.getOffsetWithClient(this.resizeColTarget, 'left');
                    const thOpt = this.getThOpt(this.resizeColTarget);
                    if (thOpt) {
                        if (thOpt.fixed === 'right') {
                            const diff = evt.clientX - thOffsetLeft;
                            thOpt.width = (thOpt.width || this.resizeColTarget.offsetWidth) - diff;
                        } else {
                            const diff = evt.clientX - (thOffsetLeft + this.resizeColTarget.offsetWidth);
                            thOpt.width = (thOpt.width || this.resizeColTarget.offsetWidth) + diff;
                        }
                        if (thOpt.width < 50) {
                            thOpt.width = 50;
                        }
                        this.$emit('on-col-resize', thOpt);
                        this.handleResize();
                        this.$nextTick(() => {
            ...
            handleTHeadMousedown(evt) {
                if (this.canResizeCol) {
                    this.isOnResizeCol = true;
                    this.setResizeLinePos(evt);
                }
            },
            handleTHeadMousemove(evt) {
                if (this.canResizeCol && this.isOnResizeCol) {
                    this.setResizeLinePos(evt);
                    return;
                }
                const diff = 6;
                const targetTh = this.getTh(evt.target);
                if (!targetTh) {
                    this.resetResizeCol();
                    return;
                }
                if (!this.resizeColTarget) {
                    /* 默认尾部拖动 */
                    if (!this.isFixedRightTh(targetTh) &&
                        Math.abs(targetTh.offsetWidth - evt.offsetX) < diff) {
                        this.resizeColTarget = targetTh;
                    } else {
                        /* 右侧固定，从头部拖动 */
                        const thOffsetLeft = this.getOffsetWithClient(targetTh, 'left');
                        if (Math.abs(evt.clientX - thOffsetLeft) < diff) {
                            this.resizeColTarget = targetTh;
                        } else {
                            this.resizeColTarget = null;
                        }
                ...
                const lastTh = this.resizeColTarget;
                const lastThOffsetLeft = this.getOffsetWithClient(lastTh, 'left');
                if ((this.isFixedRightTh(targetTh) && evt.offsetX < diff) ||
                    Math.abs((lastThOffsetLeft + lastTh.offsetWidth) - evt.clientX) < diff
                ) {
                    this.canResizeCol = true;
                    this.toggleCursor('col-resize');
                } else {
                    this.resetResizeCol();
                }
            },
